...
if(args[3] == "windows"){
	for(let i = 0 ; i < count; i++){
		command += `cd "${ args[6 + i] }"; git remote update origin --prune; git branch --merged ${args[4]} | ForEach-Object {echo $_; if($_ | Select-String -Pattern "${regexString}"){echo nope} else {git branch $_.Trim() -d}};`
	}
	fs.writeFileSync('./script.ps1', command);
}
else if(args[3] == "linux"){
	for(let i = 0 ; i < count; i++){
		command += `cd "${ args[6 + i] }"; git remote update origin --prune; git branch --merged ${args[4]} | grep -v '${regexString}' | xargs git branch -d;`
	}
